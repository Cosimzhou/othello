// ImprovedEvaluation.cpp: implementation of the CImprovedEvaluation class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "Othello.h"
#include "ImprovedEvaluation.h"
...
const int CImprovedEvaluation::midBoard[midBoardLength] = { 
	21, 22, 23, 24, 29, 30, 31, 32, 33, 34, 38, 39, 40, 41, 42, 43, 
	47, 48, 49, 50, 51, 52, 56, 57, 58, 59, 60, 61, 66, 67, 68, 69
};

const int CImprovedEvaluation::weight_train[midGameLength][IND_NUM]= {
...
int CImprovedEvaluation::score_edge[lineStateNUM][midGameLength];

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CImprovedEvaluation::CImprovedEvaluation()
{
	Initial();
}
...
bool CImprovedEvaluation::edge_IsStab(int* edge, int sq, int color){
	int oppcolor = Oppl_ch(color), off=1;
	bool flag = false,dir[2]={true,true}, 
		emp[2]={false,false}, opp[2]={false,false};
	if ((sq == 7) || (sq == 0))	return true;	//at the start or end point is stable
	if (edge[7] == color || edge[0] == color){			//if start and end point is not occupied by same color, is non-stable
		do{	
			if(dir[0]){
				if(sq+off>7)dir[0]=false;
				if(edge[sq+off] == EMPTY_IN_TEMPLATE){
					dir[0]=false;
					emp[0]=true;
				}else if(edge[sq+off] == oppcolor){
					dir[0]=false;
					opp[0]=true;
				}
			}
			if(dir[1]){
				if(sq-off<0)dir[1]=false;
				if(edge[sq-off] == EMPTY_IN_TEMPLATE){
					dir[1]=false;
					emp[1]=true;
				}else if(edge[sq-off] == oppcolor){
			...
			off++;	//go to end point, if there are all same color, stable
		}while (dir[0] || dir[1]);
		return ((emp[0]&&(emp[1]||opp[1]))||emp[1]&&opp[0]);
	}
	return false;
}

int CImprovedEvaluation::get_score_edge(int* edge, int emptiesIndex, int color){
...
int CImprovedEvaluation::get_score_edge_all(ChessType* board, ChessType color, int empties)
{
	int sum = 0;
...
int CImprovedEvaluation::getDeltaKeyano(ChessType* board, ChessType color, ChessType oppcolor)
{
	int sum = 0;
...
int CImprovedEvaluation::getUnStab(ChessType* board, ChessType color){
	int sum = 0, num2 = 0;
...
/*
int CImprovedEvaluation::getUnStab_Start(ChessType* board, ChessType color)
{
	int num = 0;
	if (((board[10] != color) && (board[11] == color)) && ((((board[0x11] != color) || (board[12] != color)) || ((board[13] != color) || (board[14] != color))) || ((board[15] != color) || (board[0x10] != color))))

		num++;
	if (((board[0x11] != color) && (board[0x10] == color)) && ((((board[10] != color) || (board[11] != color)) || ((board[12] != color) || (board[13] != color))) || ((board[14] != color) || (board[15] != color))))
		num++;
	if (((board[10] != color) && (board[0x13] == color)) && ((((board[0x49] != color) || (board[0x1c] != color)) || ((board[0x25] != color) || (board[0x2e] != color))) || ((board[0x37] != color) || (board[0x40] != color))))
		num++;
	if (((board[10] != color) && (board[20] == color)) && ((((board[80] != color) || (board[30] != color)) || ((board[40] != color) || (board[50] != color))) || ((board[60] != color) || (board[70] != color))))
		num++;
	if (((board[0x11] != color) && (board[0x19] == color)) && ((((board[0x49] != color) || (board[0x21] != color)) || ((board[0x29] != color) || (board[0x31] != color))) || ((board[0x39] != color) || (board[0x41] != color))))
		num++;
	if (((board[0x11] != color) && (board[0x1a] == color)) && ((((board[80] != color) || (board[0x23] != color)) || ((board[0x2c] != color) || (board[0x35] != color))) || ((board[0x3e] != color) || (board[0x47] != color))))
		num++;
	if (((board[0x49] != color) && (board[0x40] == color)) && ((((board[10] != color) || (board[0x13] != color)) || ((board[0x1c] != color) || (board[0x25] != color))) || ((board[0x2e] != color) || (board[0x37] != color))))
		num++;
	if (((board[0x49] != color) && (board[0x41] == color)) && ((((board[0x11] != color) || (board[0x19] != color)) || ((board[0x21] != color) || (board[0x29] != color))) || ((board[0x31] != color) || (board[0x39] != color))))
		num++;
	if (((board[80] != color) && (board[70] == color)) && ((((board[10] != color) || (board[20] != color)) || ((board[30] != color) || (board[40] != color))) || ((board[50] != color) || (board[60] != color))))
		num++;
	if (((board[80] != color) && (board[0x47] == color)) && ((((board[0x11] != color) || (board[0x1a] != color)) || ((board[0x23] != color) || (board[0x2c] != color))) || ((board[0x35] != color) || (board[0x3e] != color))))
		num++;
	if (((board[0x49] != color) && (board[0x4a] == color)) && ((((board[80] != color) || (board[0x4b] != color)) || ((board[0x4c] != color) || (board[0x4d] != color))) || ((board[0x4e] != color) || (board[0x4f] != color))))
		num++;
	if (((board[80] != color) && (board[0x4f] == color)) && ((((board[0x49] != color) || (board[0x4a] != color)) || ((board[0x4b] != color) || (board[0x4c] != color))) || ((board[0x4d] != color) || (board[0x4e] != color))))
		num++;
	return num;
}
*/

int CImprovedEvaluation::MidEval2(ChessType* board, ChessType color, ChessType oppcolor, int empties, Empties *EmHead)
{
...
int CImprovedEvaluation::StaEval2(ChessType* board, ChessType color, ChessType oppcolor, int empties, Empties* EmHead)
{
	int square, sum = 0, myDISC = 0, opDISC = 0, myMOB = 0, opMOB = 0;
	int potMobility = 0, oppPotMobility = 0, mySTAB = 0, opSTAB = 0;
	int delta = 0, unSTAB = 0, emptiesIndex = empties - 10;
...
void CImprovedEvaluation::Initial(){
	int edge[8];
	InitRecursion(edge, 0);
}

void CImprovedEvaluation::InitRecursion(int *edge, int depth)
{
	if(depth==8){
		int sum = edge[0];
